...
public class ApoliEntityConditions {
	...
	public static final RegistryObject<SingleFieldEntityCondition<Optional<ConfiguredItemCondition<?, ?>>>> USING_ITEM = register("using_item", CalioCodecHelper.optionalField(ConfiguredItemCondition.CODEC, "item_condition"), (entity, configuration) -> entity.isUsingItem() && configuration.map(x -> x.check(entity.getItemInHand(entity.getUsedItemHand()))).orElse(true));
